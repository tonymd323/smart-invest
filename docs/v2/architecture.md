...
_版本：v2.5 | 日期：2026-03-28 | 超预期算法重构：多年预期 + 采集层职责分离_
...
| ConsensusProvider | 东方财富 F10 | AkShare | ✅ fetch_and_apply_consensus | ✅ 生产 |
...
- 数据质量校验
- `fetch_and_apply_consensus()` → AkShare 获取多年一致预期（25E/26E/27E）→ 写 consensus 表 → 按报告期匹配预期年份 → 计算 expectation_diff_pct
...
| consensus | 增长中 | 多年一致预期（AkShare 增长对比表）UNIQUE(stock_code, year) |
| prices | 480 | 日K行情 |
...
_架构 v2.5 | 2026-03-28 19:58 | 超预期算法重构：AkShare多年预期 + 采集层职责分离_
