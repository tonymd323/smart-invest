...
| 2 | 📌 今日行动 | `/action` | 综合研判操作建议（信号×持仓×行情×回调评分 → 买什么/什么价买/什么价卖）| ✅ v2.9 重构完成 |
| 3 | 📋 信号看板 | `/signals` | 原始信号筛选排序 | ✅ |
| 4 | 🔍 发现池 | `/discovery` | 自动发现候选股 | ✅ |
| 5 | 📰 事件流 | `/events` | 事件时间线（待接入新闻） | ⏸️ 框架保留 |
| 6 | 📈 T+N 跟踪 | `/tracking` | 入池后收益跟踪 | ✅ |
| 7 | 📊 策略回测 | `/backtest` | 历史信号收益 | ✅ |
| 8 | ⚙️ 系统控制 | `/system` | Pipeline 触发 + Cron | ✅ |
| 9 | 📒 持仓管理 | `/portfolio` | 跟踪池增删改查 + 发现池升级 | ⏳ 新增 |
...
_架构 v2.9 | 2026-03-29 | 今日行动页重构完成 + HTMX/Tailwind 本地化_
