...
_版本：v2.15 | 日期：2026-03-29 | Docker 化部署 + 跟踪页增强 + 数据质量修复_
...
### v2.15 新增问题（2026-03-29 Docker化后发现）

| ID | 问题 | 优先级 | 说明 |
|----|------|--------|------|
| D-15 | stocks 表覆盖不全 | P0 | 只有 430 只（创业板为主），缺失上证+科创板+北交所 |
| D-16 | 北交所名称缺失 | P1 | Pipeline 写入时查 stocks 表，查不到就用代码当名称 |
| D-17 | 发现池收录旧报告 | P1 | Q3 2025 报告被扫入，未过滤报告期 |
| D-18 | event_tracking 缺报告期 | P1 | create_tn_tracking 不带 report_period/actual_yoy |

### v2.15 修复状态（2026-03-29）

| ID | 问题 | 状态 | 说明 |
|----|------|------|------|
| D-15 | stocks 表覆盖不全 | ⏳ 待修 | 需写全量 A 股扩充脚本 |
| D-16 | 北交所名称缺失 | ✅ 已修 | 回调脚本从东财 API 补填，Pipeline 加 Tushare 兜底 |
| D-17 | 发现池收录旧报告 | ✅ 已修 | _write_discovery_pool 过滤 report_date < 2025-10 |
| D-18 | event_tracking 缺报告期 | ✅ 已修 | create_tn_tracking 从 discovery_pool 读取并写入 |
...
## v2.13 需求 — 前端体验重构（UX v2）

### 背景
前端 v1.0（FastAPI + Jinja2 + HTMX + Tailwind）功能齐全但用户体验存在系统性问题：
1. **总览页信息层级混乱** — 三栏并列（信号/持仓/系统健康），权重相同，系统健康不该和持仓并列
2. **所有页面都是表格** — 信号看板、发现池、持仓管理、事件流四个不同业务场景，呈现方式几乎一样
3. **缺乏视觉差异** — 用户区分页面只能靠标题，认知负担高
4. **决策按钮无 loading 反馈** — 点击后无视觉反馈，误操作风险

### 设计原则
- **扫一眼就懂：** 关键信息用大号数字+颜色，不用逐行读表格
- **场景化呈现：** 不同业务场景用不同布局（卡片网格/时间线/仪表盘）
- **移动端优先：** 不是把桌面表格缩小，而是移动端独立的卡片式 feed
- **操作确认：** 重要操作（卖出）加确认，防止误触

### 需求清单

#### A. 总览页重构（P0 🔴）
**现状：** 三栏并列（信号摘要/持仓/系统健康），信号摘要只有三个数字
**目标：** 四层信息层级

| 层级 | 内容 | 设计 |
|------|------|------|
| 第一层 | 一句话状态摘要 | 顶部横幅：「今日2只超预期，福耀回调到位」，无信号时灰色 |
| 第二层 | 今日行动预览 | 最关键2-3条行动卡片，链接到今日行动页 |
| 第三层 | 持仓快照 | 每只股票一张 mini 卡片：名称+现价+涨跌+盈亏条 |
| 第四层 | 折叠区 | 信号趋势图 + 最近事件（可展开/收起） |

#### B. 发现池改为卡片网格（P0 🔴）
**现状：** 表格（股票名/来源/评分/信号/发现时间）
**目标：** 每只股票一张卡片，包含评分条（可视化进度条）、来源标签、一键升级按钮

#### C. 事件流改为时间线布局（P1 🟡）
**现状：** 平铺卡片列表
**目标：** 左侧时间线 + 右侧内容，时间点标记利好/利空色彩

#### D. 决策按钮交互增强（P1 🟡）
**现状：** 点击后 disabled，无 loading/success 反馈
**目标：** 点击→spinner→1秒内显示✅；卖出按钮加二次确认

#### E. 持仓卡片化（P2 🟢）
**现状：** 表格（8列数据）
**目标：** 关键数字大号显示（市值/盈亏金额），盈亏进度条可视化

#### F. 移动端信号卡片化（P2 🟢）
**现状：** 表格横向滚动
**目标：** 卡片式 feed，像新闻客户端一样滚动

### 验收标准
- [ ] 总览页四层信息层级，一眼看到今日该关注什么
- [ ] 发现池卡片网格，评分可视化
- [ ] 事件流时间线布局
- [ ] 决策按钮有 loading + success 反馈
- [ ] 移动端全页面卡片化（非表格缩小）
- [ ] 既有 T1-T25 回归通过
...
**架构：** Python 3.11-slim + 清华 PyPI 镜像源 + starlette<1.0 + python-multipart

| 组件 | 配置 |
|------|------|
| 镜像 | smart-invest:latest |
| 端口 | 8080 |
| Volume | ./data → /app/data, ./config → /app/config |
...
```bash
cd /root/.openclaw/workspace/smart-invest
git pull && docker compose up -d --build
```

### Cron 任务（宿主机 docker exec 调度）

| 时间 | 任务 | 命令 |
|------|------|------|
| 15:35 周一~五 | 收盘扫描 | docker exec smart-invest python3 scripts/run_pipeline.py --window 6h --max-stocks 300 |
| 21:00 周一~五 | 晚间全量 | docker exec smart-invest python3 scripts/run_pipeline.py --window 6h |
| 周日 10:00 | BTIQ 回填 | docker exec smart-invest python3 scripts/btiq_backfill.py |

### 数据维护

- **回调脚本：** `scripts/fix_tracking_data.py` — 补填北交所名称+财务数据+过滤新股/B股
- **执行方式：** `docker exec smart-invest python3 scripts/fix_tracking_data.py`
- **建议频率：** 每周一次
