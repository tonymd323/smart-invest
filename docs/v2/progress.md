...
| 任务 | 预估 | 说明 |
|------|------|------|
| 路由+模板 | 0.5h | `/portfolio` 页面骨架 |
| stocks.json 读写 | 1h | get/update stocks.json，支持增删改 |
| HTMX 表单交互 | 1h | 在线编辑成本价/目标价/止损价 |
| 发现池升级入口 | 0.5h | 一键从发现池升级到跟踪池 |

### 今日行动重构 ⏳（~2h）

| 任务 | 预估 | 说明 |
|------|------|------|
| 持仓决策模块 | 0.5h | 持有股票最新信号→加仓/减仓/持有/卖出 |
| 买入建议模块 | 0.5h | 发现池候选→目标价+止损价+评分 |
| 关注提醒模块 | 0.5h | 值得关注但未到买入时机 |
| 模板重构 | 0.5h | 从信号列表→操作建议卡片 |

### 全局列表增强 ⏳（~4.5h）

| 任务 | 预估 | 说明 |
|------|------|------|
| 通用 list_controls 宏 | 1h | Jinja2 模板宏：搜索框+筛选+排序+分页导航 |
| 后端分页通用函数 | 0.5h | services.py paginate_query() 封装 |
| 持仓管理发现池改造 | 0.5h | 接入通用组件 |
| 信号看板改造 | 0.5h | 接入通用组件 |
| 发现池改造 | 0.5h | 接入通用组件 |
| 事件流/T+N/回测改造 | 1h | 3 个页面统一接入 |
| 测试验证 | 0.5h | — |

### ConsensusProvider 多源从严 ✅（已完成）

| 任务 | 预估 | 说明 |
|------|------|------|
| DB 迁移（source_detail 字段） | 5 min | ALTER TABLE consensus |
| ConsensusProvider 重构 | 30 min | 并行取两源 + 取 max |
| Pipeline 适配 | 10 min | 写入 source_detail |
| 测试用例 | 20 min | 两源对比 + 回归 |
| 验证 + 提交 | 10 min | — |
...
2. ✅ services.py 查询层：解析 summary JSON/dict → 可读中文文本（兼容双引号JSON和单引号Python dict）
3. ✅ services.py 查询层：英文 → 中文映射（analysis_type/signal/source/event_type/tracking_status）
4. ✅ services.py 查询层：LEFT JOIN stocks 补全公司名称（修复 et.* 与 COALESCE 列名冲突）

**P1（已完成）：**
5. ✅ 移动端响应式改造（汉堡菜单 + 抽屉导航 + 遮罩层 + 表格横向滚动 + 网格自适应）
6. ⏳ earnings 表加查询过滤（quarterly_net_profit > 0）— 后端已修，前端暂未加过滤

**P2（后续）：**
7. ConsensusProvider 预注入配置文件支持
8. Plotly 图表集成（T+N 曲线 + 回测柱状图）

### Git 提交
- `2814fd3` fix: 前端5项修复 — 清理N/A脏数据 + 中文映射 + JSON格式化 + JOIN补全名称
- `c1ba0c3` feat: 移动端响应式适配
- `94f9f18` refactor: ConsensusProvider 多源从严 — AkShare多年预期 + Provider模式
- `4a1dcdd` docs: ConsensusProvider 多源从严 — PRD/架构/进度三文件更新
- `155e72e` feat: 今日行动页重构 v2.9 — 综合研判操作建议
- `84a1aea` feat: 持仓管理页面 — 跟踪池/关注池/发现池升级
- `4794ef1` fix: 决策按钮无响应 — API返回HTMLResponse
- `2d2cf1f` fix: 按钮vanilla JS + 股票名称修复
- `b63efb3` docs: 决策流转设计(v2.10待做)
...
## v2.13 开发计划 — 前端体验重构（UX v2）

### 背景
2026-03-29 12:44 产品设计（小达）完成前端全面 UX 评审，发现 6 个系统性问题。详见 PRD v2.8 v2.13 需求变更。

### Phase D: 总览页重构（~3h）🔴 P0

| # | 任务 | 文件 | 预估 | 说明 |
|---|------|------|------|------|
| D-1 | 一句话状态摘要横幅 | web/templates/dashboard.html | 0.5h | 顶部：今日信号数量 + 关键事件文字，有信号时红/绿色 |
| D-2 | 今日行动预览卡片 | web/templates/dashboard.html + services.py | 1h | 最关键2-3条行动卡片，链接到 /action |
| D-3 | 持仓 mini 卡片 | web/templates/dashboard.html | 0.5h | 每只股票一行：名称+现价+涨跌+盈亏金额 |
| D-4 | 图表+事件折叠区 | web/templates/dashboard.html | 0.5h | 默认收起，点击展开信号趋势图+最近事件 |
| D-5 | 移除系统健康卡片 | web/templates/dashboard.html | 0.5h | 移到 /system 页面，总览不再显示 DB 统计 |

### Phase E: 发现池卡片网格（~2h）🔴 P0

| # | 任务 | 文件 | 预估 | 说明 |
|---|------|------|------|------|
| E-1 | 卡片网格模板 | web/templates/discovery.html | 1h | 每只股票一张卡片：评分条+信号标签+来源+升级按钮 |
| E-2 | CSS 卡片样式 | web/static/style.css | 0.5h | .discovery-card + 评分进度条 + 信号色彩 |
| E-3 | 响应式适配 | web/static/style.css | 0.5h | 桌面3列→平板2列→手机1列 |

### Phase F: 事件流时间线（~2h）🟡 P1

| # | 任务 | 文件 | 预估 | 说明 |
|---|------|------|------|------|
| F-1 | 时间线模板 | web/templates/events.html | 1h | 左侧日期线+时间点，右侧事件卡片 |
| F-2 | CSS 时间线样式 | web/static/style.css | 0.5h | .timeline + .timeline-node + 利好/利空色彩 |
| F-3 | 日期分组逻辑 | web/services.py | 0.5h | 按日期分组，组内按时间倒序 |

### Phase G: 决策按钮交互增强（~1h）🟡 P1

| # | 任务 | 文件 | 预估 | 说明 |
|---|------|------|------|------|
| G-1 | loading spinner | web/templates/action.html | 0.3h | 点击→spinner CSS动画→禁用全部按钮 |
| G-2 | success 反馈 | web/templates/action.html | 0.3h | API成功→显示✅+文字确认，1.5秒后消失 |
| G-3 | 卖出二次确认 | web/templates/action.html | 0.4h | modal弹窗：「确认卖出 福耀玻璃 ¥57.85？」 |

### Phase H: 持仓卡片化 + 移动端卡片化（~3.5h）🟢 P2

| # | 任务 | 文件 | 预估 | 说明 |
|---|------|------|------|------|
| H-1 | 持仓卡片布局 | web/templates/portfolio.html | 1h | 大号数字（市值/盈亏金额），盈亏进度条 |
| H-2 | 移动端信号卡片 | web/templates/signals.html | 1h | <768px 时表格→卡片 feed（非横向滚动） |
| H-3 | CSS 响应式统一 | web/static/style.css | 1h | 全页面移动端卡片化样式 |
| H-4 | Plotly 移动端适配 | web/static/chart-config.js | 0.5h | 图表高度+字号自动调整 |

### 开发顺序

```
D-1→D-2→D-3→D-4→D-5（总览重构）
→ E-1→E-2→E-3（发现池卡片）
→ F-1→F-2→F-3（事件流时间线）
→ G-1→G-2→G-3（决策按钮）
→ H-1→H-2→H-3→H-4（持仓+移动端卡片化）
```
...
| M | 内容 | 依赖 |
|---|------|------|
| M6 | 总览页四层信息层级可用 | D-1~D-5 |
| M7 | 发现池卡片网格可用 | E-1~E-3 |
| M8 | 事件流时间线可用 | F-1~F-3 |
| M9 | 决策按钮交互增强 | G-1~G-3 |
| M10 | 移动端全页面卡片化 | H-1~H-4 |
| M11 | 全量 CSS 统一美化 | M6~M10 并行 |

### 关键文件清单

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| `web/templates/dashboard.html` | 重写 | 四层仪表盘布局 |
| `web/templates/discovery.html` | 重写 | 卡片网格替代表格 |
| `web/templates/events.html` | 重写 | 时间线布局 |
| `web/templates/action.html` | 增强 | loading/success/confirm 交互 |
| `web/templates/portfolio.html` | 美化 | 持仓卡片化 |
| `web/templates/signals.html` | 增强 | 评分条可视化 + 移动端卡片化 |
| `web/static/style.css` | 扩展 | 卡片网格/时间线/评分条/移动端样式 |
| `web/static/chart-config.js` | 微调 | 移动端图表适配 |
| `web/services.py` | 增强 | get_dashboard_summary() 重构 + 事件日期分组 |
